-- Manual deployment script for prompt updates
-- Execute this directly in Supabase SQL Editor to update prompts with Persian translation support

-- First, let's check current prompts
SELECT 'Current prompts before update:' as info;
SELECT prompt_type, prompt_name, version, is_latest, is_active FROM prompts ORDER BY prompt_type, version;

-- Update Educational prompt
...
-- Verify the updates
SELECT 'Updated prompts:' as info;
SELECT 
  prompt_type,
  prompt_name,
  version,
